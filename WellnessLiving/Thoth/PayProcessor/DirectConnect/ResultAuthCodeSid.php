...
class ResultAuthCodeSid
{
  ...
  /**
   * Stop Recurring Payment. Stop all future Payments. See {@link https://developer.visa.com/request_response_codes}
   *  "Action Code (actionCode)" section and {@link Thoth/PayProcessor/DirectConnect/doc/test-data-samples-2018.pdf}
   *  "Section 1: Standard Credit Card Transactions (Sierra Driver Triggers)" section.
   *
   * Cardholder has revoked authorization for recurring transactions.
   */
  const STOP_RECURRING = 'R1';

  /**
   * @inheritDoc
   */
  const _TRANSLATION_CONTEXT = 'Wl.Pay.Processor.DirectConnect.ResultAuthCodeSid';
}
