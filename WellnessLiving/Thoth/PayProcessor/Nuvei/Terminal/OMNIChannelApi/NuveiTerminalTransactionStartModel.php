 ...
 *
 * @method WlModelRequest post() Starts a payment transaction on a `Nuvei` POS terminal.  Validates the terminal, business merchant, and staff permissions, then creates a pay  transaction and sends a payment request to the terminal. If another transaction is already  in progress on the terminal, attempts to recover or cancel it. Schedules a background task to  poll the terminal for the swipe result and a fallback task to void the transaction if it is  never completed.
 */
