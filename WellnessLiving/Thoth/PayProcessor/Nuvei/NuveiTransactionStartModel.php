 ...
 *
 * @method WlModelRequest post() Starts a `Nuvei` card authorization transaction for a purchase or a card verification.  Validates the submitted card form (either a new card or a previously stored one), creates a  pending pay transaction, obtains a `Nuvei` session token, and requests the card  authorization or, when only verifying the card, a zero-amount authorization. On success,  stores a `Nuvei` 3-D Secure payment session for a possible follow-up step and schedules a  task to void the transaction automatically if it is never completed. The transaction is  finalized with the processor result regardless of the outcome.
 */
 ...
 *       <dd>Key of the country. Required for newly added billing address.
 * </dd>
 * 
 *       <dt>string `k_geo_region`</dt>
 *       <dd>Key of the region. Required for newly added billing address.
 * </dd>
 * 
 *       <dt>string|null `k_pay_address`</dt>
 *       <dd>
 *         Chosen payment address key.
 * It will be set even if user decided to add new address.
 * 
 *       </dd>
 * 
 *       <dt>string `s_name`</dt>
 *       <dd>User name. Required for newly added billing address.</dd>
 * 
 ...
 *   <dd>Key of existing payment source in case of editing.
 * </dd>
 * 
 *   <dt>string `s_name`</dt>
