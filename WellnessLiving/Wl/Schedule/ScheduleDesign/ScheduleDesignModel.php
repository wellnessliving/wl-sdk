...
use WellnessLiving\Wl\Schedule\Design\CellSid;
use WellnessLiving\Wl\Schedule\Design\IntervalSid;
use WellnessLiving\Wl\Schedule\Design\OptionSid;
use WellnessLiving\Wl\Schedule\Design\WeekDaySid;
 ...
 *
 * @method WlModelRequest get() This method is called to process GET query.  Loads the schedule design settings configured for the business, including cell size, time  interval, first day of week, and the colors and toggles that control what is shown on the  staff schedule (staff busy diagonal lines, drag-and-drop, cancelled appointments/classes,  notes, and icons). Access is public when no user key is given, because the schedule design  also drives public schedule widgets that guests can view; when a user key is given, the  user's profile access to the business is checked.
 * @method WlModelRequest post() Saves schedule design settings including schedule config settings.  Validates that the current user has permission to edit the business schedule design, then  merges the given fields into the existing schedule design and schedule config records for  the business and user, creating them when they do not exist yet. Saves the updated schedule  design row and the schedule config's serialized settings, refreshes the related caches,  marks the config as the user's default filter when requested, and logs the change when any  setting was actually modified.
 */
class ScheduleDesignModel extends WlModelAbstract
{
  /**
   * Schedule time interval in minutes. One of {@link IntervalSid} constants. 0 if not set yet.
   *
   * @get result
   * @post post
   * @var int
  ...
  /**
   * Cell size. One of {@link CellSid} constants. 0 if not set yet.
   *
   * @get result
   * @post post
   * @var int
  ...
  /**
   * Option of appointments display. One of {@link OptionSid} constants. 0 if not set yet.
   *
   * @get result
   * @post post
   * @var int
   ...
   *
   * Constant from {@link WeekDaySid}. 0 if not set yet.
   *
   * @get result
   * @post post
   * @var int
